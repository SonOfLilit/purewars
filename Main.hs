...
type Lines = [(GL.GLfloat, GL.GLfloat, GL.GLfloat, GL.GLfloat)]
data GameState = GameState {x, y :: GL.GLfloat}
...
logic t = do
  ...
  put state {x = x state + 1*d, y = y state + 2*d}

getLines :: GameState -> Lines
getLines GameState {x = x, y = y} = [
  (x, y, 0.0, 30.0),
  (0.0, 30.0, 30.0, 30.0),
  (30.0, 30.0, x, y)]

drawOneLine :: GL.Vertex2 GL.GLfloat -> GL.Vertex2 GL.GLfloat -> IO ()
...
drawLines ls = do
  ...
  forM_ ls (\ (x0, y0, x1, y1) ->  
                drawOneLine (GL.Vertex2 x0 y0) (GL.Vertex2 x1 y1))
...
main = do
  ...
  now <- getCurrentTime
  stateRef <- newIORef GameState {x = 0, y = 8}
